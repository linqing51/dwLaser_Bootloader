...
#define SET_LAS_FAN(b)								HAL_GPIO_WritePin(LAS_FAN_GPIO_Port, LAS_FAN_Pin, b)
#define FLIP_LAS_FAN()								HAL_GPIO_TogglePin(LAS_FAN_GPIO_Port, LAS_FAN_Pin)
...
#define SET_SPK_EN(b)								HAL_GPIO_WritePin(SPK_EN_GPIO_Port, SPK_EN_Pin, b)
#define FLIP_SPK_EN()								HAL_GPIO_TogglePin(SPK_EN_GPIO_Port, SPK_EN_Pin)

#define SET_RED_LED(b)								HAL_GPIO_WritePin(RED_LED_GPIO_Port, RED_LED_Pin, b)
#define FLIP_RED_LED()								HAL_GPIO_TogglePin(RED_LED_GPIO_Port, RED_LED_Pin)

#define SET_GREEN_LED(b)							HAL_GPIO_WritePin(GREEN_LED_GPIO_Port, GREEN_LED_Pin, b)
#define FLIP_GREEN_LED()							HAL_GPIO_TogglePin(GREEN_LED_GPIO_Port, GREEN_LED_Pin)

#define SET_BLUE_LED(b)							HAL_GPIO_WritePin(BLUE_LED_GPIO_Port, BLUE_LED_Pin, b)
#define FLIP_BLUE_LED()							HAL_GPIO_TogglePin(BLUE_LED_GPIO_Port, BLUE_LED_Pin)
...
#define SET_AIM_PWM(b)								HAL_GPIO_WritePin(AIM_PWM_GPIO_Port, AIM_PWM_Pin, b)
#define FLIP_AIM_PWM()								HAL_GPIO_TogglePin(AIM_PWM_GPIO_Port, AIM_PWM_Pin)
/*****************************************************************************/
typedef enum {
...
void bootLoadInit(void){//引导程序初始化
	...
	//R-G-B流水
	//R
	SET_RED_LED(GPIO_PIN_SET);//设置R LED亮度
	SET_GREEN_LED(GPIO_PIN_RESET);//设置G LED亮度
	SET_BLUE_LED(GPIO_PIN_RESET);//设置B LED亮度
	HAL_Delay(500);
	//G
	SET_BLUE_LED(GPIO_PIN_RESET);//设置B LED亮度
	SET_RED_LED(GPIO_PIN_RESET);//设置R LED亮度
	SET_GREEN_LED(GPIO_PIN_SET);//设置G LED亮度
	HAL_Delay(500);
	//B
	SET_BLUE_LED(GPIO_PIN_SET);//设置B LED亮度
	SET_RED_LED(GPIO_PIN_RESET);//设置R LED亮度
	SET_GREEN_LED(GPIO_PIN_RESET);//设置G LED亮度
	HAL_Delay(500);
	SET_BLUE_LED(GPIO_PIN_RESET);//设置B LED亮度
	...
	printf("\r\n");   
	//显示输入IO状态
	if(HAL_GPIO_ReadPin(ESTOP_NC_GPIO_Port, ESTOP_NC_Pin) == GPIO_PIN_SET){
		printf("Bootloader:INPUT->ESTOP_NC      = Open!\n");
	}
	else{
		printf("Bootloader:INPUT->ESTOP_NC      = Close!\n");
	}
	if(HAL_GPIO_ReadPin(FS_NC_GPIO_Port, FS_NC_Pin) == GPIO_PIN_SET){
		printf("Bootloader:INPUT->FSWITCH_NC    = Open!\n");
	}
	else{
		printf("Bootloader:INPUT->FSWITCH_NC    = Close!\n");
	}
	if(HAL_GPIO_ReadPin(FS_NO_GPIO_Port, FS_NO_Pin) == GPIO_PIN_SET){
	...
	if(HAL_GPIO_ReadPin(LAS_PWM1_GPIO_Port, LAS_PWM1_Pin) == GPIO_PIN_SET){//LPA_PWM1
	...
	if(HAL_GPIO_ReadPin(LAS_FAN_GPIO_Port, LAS_FAN_Pin) == GPIO_PIN_SET){//FAN
		printf("Bootloader:OUTPUT->LAS_FAN      = High!\n");
	}
	else{
		printf("Bootloader:OUTPUT->LAS_FAN    	= Low!\n");
	}
	if(HAL_GPIO_ReadPin(LAS_TEC_GPIO_Port, LAS_TEC_Pin) == GPIO_PIN_SET){//TEC
		printf("Bootloader:OUTPUT->LAS_TEC      = High!\n");
	}
	else{
...
static void beepDiag(uint8_t diag){//蜂鸣器诊断声音 摩尔斯电码
	...
	switch(diag){
		case '0':{
			//-
			SET_RED_LED(GPIO_PIN_SET);HAL_Delay(MORSECODE_SHORT_TIME);SET_RED_LED(GPIO_PIN_RESET);HAL_Delay(MORSECODE_SPACE_TIME);
			//-
			SET_RED_LED(GPIO_PIN_SET);HAL_Delay(MORSECODE_LONG_TIME);SET_RED_LED(GPIO_PIN_RESET);HAL_Delay(MORSECODE_SPACE_TIME);
			//-
			SET_RED_LED(GPIO_PIN_SET);HAL_Delay(MORSECODE_LONG_TIME);SET_RED_LED(GPIO_PIN_RESET);HAL_Delay(MORSECODE_SPACE_TIME);
			//-
			SET_RED_LED(GPIO_PIN_SET);HAL_Delay(MORSECODE_LONG_TIME);SET_RED_LED(GPIO_PIN_RESET);HAL_Delay(MORSECODE_SPACE_TIME);
			//-
			SET_RED_LED(GPIO_PIN_SET);HAL_Delay(MORSECODE_LONG_TIME);SET_RED_LED(GPIO_PIN_RESET);
			break;
		}
		case '1':{
			//.
		...
		case '5':{
			...
			SET_RED_LED(GPIO_PIN_SET);HAL_Delay(MORSECODE_SHORT_TIME);SET_RED_LED(GPIO_PIN_RESET);HAL_Delay(MORSECODE_SPACE_TIME);
			//.
			SET_RED_LED(GPIO_PIN_SET);HAL_Delay(MORSECODE_SHORT_TIME);SET_RED_LED(GPIO_PIN_RESET);
			break;
		}
		case '6':{
			//-
